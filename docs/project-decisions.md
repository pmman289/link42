...
- 连接中间层插件绑定到受管连接，用于接管受管连接中的部分字段，例如 Endpoint host/port。
- 被插件接管的字段在原表单中只读展示，真实连接地址和协议参数放在插件自己的表单区域。
- 第一版连接中间层采用内置注册表，不做外部动态插件；插件规范见 `docs/connection-middleware-plugins.md`。
- 创建受管连接时用户只填写：
  - 接口名称。
