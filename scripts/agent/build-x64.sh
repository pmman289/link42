...
write_release_files() {
  ...
  link42_write_sha256 "$binary"
  link42_write_sha256 "$versioned"
  link42_write_sha256 "$platform_versioned"
  local size
  ...
  docker run --rm \
    ...
    sh -c "
      for attempt in 1 2 3; do
        apt-get update && apt-get install -y --no-install-recommends binutils && break
        if [ \"\$attempt\" = 3 ]; then
          echo 'failed to install binutils after 3 attempts' >&2
          exit 1
        fi
      ...
      python -m PyInstaller --clean --noconfirm --onefile --name '$NAME' --distpath /src/dist/agent --workpath /src/build/pyinstaller --specpath /src/build/pyinstaller --paths /src/apps/agent --paths /src/packages /src/apps/agent/agent_entry.py
    "
